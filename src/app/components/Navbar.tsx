      ...
      {!hideTopBar && (
        <div className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md border-b border-border" style={{ paddingTop: 'var(--safe-top)' }}>
          <div className="flex items-center gap-3 px-4 py-2.5">
            <img src={logoImg} alt="不上班" className="h-8 w-auto flex-shrink-0" />
            ...
            {/* Filter button — only on Home/Talents */}
          ...
          {showFilter && (
            <div className="bg-white border-b border-border shadow-lg px-4 py-4 space-y-4 animate-in slide-in-from-top-2 duration-200">
              {isTalentsPage ? (
                <>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {IDENTITIES.map(v => (
                        <button key={v} onClick={() => setFIdentity(v)} className={cn('rounded-full px-3 py-1 text-xs transition-colors', fIdentity === v ? 'bg-foreground text-background' : 'bg-secondary text-foreground')}>{v}</button>
                      ))}
                    </div>
                  </div>
                  {/* City */}
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {CITIES.map(v => (
                        <button key={v} onClick={() => setFCity(v)} className={cn('rounded-full px-3 py-1 text-xs transition-colors', fCity === v ? 'bg-foreground text-background' : 'bg-secondary text-foreground')}>{v}</button>
                      ))}
                    </div>
                  </div>
                  {/* Accept Type */}
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {ACCEPT_TYPES.map(v => (
                        <button key={v} onClick={() => setFAccept(v)} className={cn('rounded-full px-3 py-1 text-xs transition-colors', fAccept === v ? 'bg-foreground text-background' : 'bg-secondary text-foreground')}>{v}</button>
                      ))}
                    </div>
                  </div>
                </>
                ...
                <>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {REGIONS.map(r => (
                        <button key={r} onClick={() => setFRegion(r)} className={cn('rounded-full px-3 py-1 text-xs transition-colors', fRegion === r ? 'bg-foreground text-background' : 'bg-secondary text-foreground')}>{r}</button>
                      ))}
                    </div>
                  </div>
                  {/* Budget */}
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {BUDGETS.map(b => (
                        <button key={b} onClick={() => setFBudget(b)} className={cn('rounded-full px-3 py-1 text-xs transition-colors', fBudget === b ? 'bg-foreground text-background' : 'bg-secondary text-foreground')}>{b}</button>
                      ))}
                    </div>
                  </div>
                  {/* Type */}
                  <div>
                    <p className="text-xs text-muted-foreground mb-2">类型</p>
                    <div className="flex flex-wrap gap-2">
                      {TASK_TYPES.map(t => (
                        <button key={t} onClick={() => setFType(t)} className={cn('rounded-full px-3 py-1 text-xs transition-colors', fType === t ? 'bg-foreground text-background' : 'bg-secondary text-foreground')}>{t}</button>
                      ))}
                    </div>
                  </div>
                </>
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button onClick={applyFilter} className="flex-1 rounded-xl bg-primary py-2 text-sm text-primary-foreground flex items-center justify-center gap-1">
          ...
          {tabs.map(tab => {
            ...
            return (
              <Link
                ...
                to={tab.path}
                className={cn(
                  'flex-1 flex flex-col items-center rounded-xl transition-colors relative',
                  isIsland ? 'gap-0 py-1 justify-end' : 'gap-0.5 py-1',
                  !isIsland && (active ? 'text-foreground' : 'text-muted-foreground')
                )}
              >
                {isIsland ? (
                  <>
                    <div className={cn(
                      'absolute left-1/2 -translate-x-1/2 -top-7 flex h-11 w-11 items-center justify-center rounded-full shadow-lg transition-all',
                      active
                        ? 'bg-gradient-to-br from-primary to-primary/70 scale-110'
                        : 'bg-gradient-to-br from-muted-foreground/80 to-muted-foreground/50'
                    )}>
                      <Icon className="h-5 w-5 text-white" />
                    </div>
                    <span className={cn('text-[10px] mt-3.5', active ? 'text-foreground' : 'text-muted-foreground')}>
                      {tab.label}
                    </span>
                  </>
                ) : (
                  <>
                    <div className="relative">
                      ...
                      {tab.badge && tab.badge > 0 && (
                        <span className="absolute -top-1.5 -right-2.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-coral px-1 text-[9px] text-white leading-none">
